...
def test_version_error_sanity():
    ...
    assert version_error.message == "version from cr (interfaces.eda.nokia.com/v1alpha1) does not match intent version (interfaces.eda.nokia.com/v1)"
...
def test_version_missing_dependency_sanity():
    ...
    assert missing_dependency.message == "missing dependency of type Interface with name dut1-ethernet-1/3"
...
def test_version_invalid_telemetry_sanity():
    ...
    assert invalid_telemetry.message == 'invalid telemetry at path .interface{.name=="ethernet-1/3"}: "unable to infer interface ethernet-1/3'
