...
from .overlay_state import DbTopoOverlayState
...
class DbTopoOverlay:
    def __init__(self,
        ...
        db_name = DbTopoOverlay.db_name(group, version, name)
        self.db_path = f'{topo_path}.overlay{{.name=="{db_name}"}}'
    ...
    def deserialize(cls,
                    data,):
        result = cls(data['db_value']['name'], data['db_value']['group'], data['db_value']['version'], data['db_topo_path'], data['enabled'], data)
        return result
